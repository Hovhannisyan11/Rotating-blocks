...
this.style.borderRadius = '20px'; // this-y vrody c-i heta
...
function f5(){
  ...
  for(let i = 0;i<d.length;i++){
    d[i].style.background = '#' + c; // esi en guyneri pahna #02153 orinak
...
function f6(){
   for(let i = 0;i<d.length;i++){
    let t =+ d[i].getAttribute('data-tox'); // esi vercnuma data-tox-i '' -i  meji tivy
    
    
    let s =+ d[i].getAttribute('data-syun'); // esele vercnuma data-syun-i ''-i meji tivy
    if(t == s || (t+s) == 3){
